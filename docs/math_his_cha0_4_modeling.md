...
请注意这段 20 行代码里同时站着**四个死人**：牛顿（自由落体的机理）、莱布尼茨（微分）、高斯（最小二乘 + 误差假设）、柯西（梯度下降）。**这就是一次完整的数学建模**，而它跟你训练一个 GPT 的代码，逐行同构 —— 后面 0.4.5 节会把这个同构一条一条对上。
...
可视化建议：用一个 GIF 展示 8 层本轮如何画出手写体的 "π" 字 —— 观众会亲眼看到"天文学的地心说"和"傅里叶级数"和"神经网络的万能逼近"是同一件事。
